...
public class SoiCauMienNamFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    private View mView;

    private FloatingActionButton fabSoiCauMn;
    private SwipeRefreshLayout refreshLayout;
    private TextView tvDateNewest, tvSongThuLoNewest, tvBachThuLoNewest;
    private RecyclerView rcvSoiCauMn;
    private List<SoiCau> listSoiCaus, listSoiCausClone;
    private SoiCauAdapter soiCauAdapter;
    private ProgressDialog progressDialog;

    private View viewDialog;
    private AlertDialog soiCauMnDlg;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView = inflater.inflate(R.layout.fragment_manager_soicau_mn, container, false);
        initView();
        initAdapter();
        initSoiCauMnDialog();
    ...
    private void initView() {
        fabSoiCauMn = mView.findViewById(R.id.fabSoiCauMn);
        rcvSoiCauMn = mView.findViewById(R.id.rcvSoiCauMn);
        tvDateNewest = mView.findViewById(R.id.tvCreatedNewestSoiCauMn);
        tvBachThuLoNewest = mView.findViewById(R.id.tvBachThuLoNewestSoiCauMn);
        tvSongThuLoNewest = mView.findViewById(R.id.tvSongThuLoNewestSoiCauMn);
        refreshLayout = mView.findViewById(R.id.refreshSoiCauMn);
        refreshLayout.setOnRefreshListener(this);

        LinearLayoutManager layoutManager = new LinearLayoutManager(requireActivity());
        layoutManager.setOrientation(RecyclerView.VERTICAL);
        rcvSoiCauMn.setLayoutManager(layoutManager);

        progressDialog = new ProgressDialog(requireActivity());
        progressDialog.setMessage(getResources().getString(R.string.vui_long_doi) + "...");
        progressDialog.setCanceledOnTouchOutside(false);

        fabSoiCauMn.setOnClickListener(this);
    }
    ...
    private void initAdapter() {
        ...
        rcvSoiCauMn.setAdapter(soiCauAdapter);
        buildAllSoiCau();
    ...
    private void buildAllSoiCau() {
        if (NetworkUtils.haveNetwork(requireActivity())) {
            ...
            DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference().child(Const.FIREBASE_REF.SOI_CAU_MN);
    ...
    private void initSoiCauMnDialog() {
        soiCauMnDlg = new AlertDialog.Builder(requireActivity(), R.style.CustomAlertDialog).create();
        viewDialog = getLayoutInflater().inflate(R.layout.dialog_new_soicau_mn, null);

        tvDateCurrentDlg = viewDialog.findViewById(R.id.tvDateTimeSoiCauMn);
        ...
        soiCauMnDlg.setView(viewDialog);
    ...
    private void onClickFabSoiCauMn() {
        clearDataDialog();
        showSoiCauMnDialog();
    }
    ...
    private void onClickAgreeDialog() {
        ...
        if (NetworkUtils.haveNetwork(requireActivity())) {
            ...
            final SoiCau soiCauCreated = SoiCau.builder()
                    ...
                    .mien(Const.SOI_CAU_BA_MIEN.MIEN_NAM)
            ...
            final DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference(Const.FIREBASE_REF.SOI_CAU_MN);
    ...
    private void showSoiCauMnDialog() {
        if (soiCauMnDlg != null && !soiCauMnDlg.isShowing()) {
            soiCauMnDlg.show();
        }
    }

    private void hiddenSoiCauMnDialog() {
        if (soiCauMnDlg != null && soiCauMnDlg.isShowing()) {
            soiCauMnDlg.dismiss();
        }
    }
